...
public abstract class EventTriggerObjectManager<T extends EventTrigger> extends AbstractObjectManager<T, T> {
    ...
    public T create(Storable parent, String type) {
        Class<? extends EventTrigger> triggerClass = subclasses.get(type);
        if (triggerClass == null) {
            throw new IllegalArgumentException("Cannot create trigger of type " + type);
        }
         try {
             EventTrigger result = (T) triggerClass.newInstance();
             result.setParent(parent);
             result = repository.save((T) result);
             return (T) result;
        } catch (InstantiationException | IllegalAccessException e) {
            throw new RuntimeException(
                    "Cannot create trigger of type " + type + " with class " + triggerClass.getCanonicalName(), e);
        }
    }
    ...
    @PostConstruct
    protected void init() {
        subclasses = new HashMap<String, Class<? extends EventTrigger>>() {
            {
                put(OperationEventTrigger.TYPE, OperationEventTrigger.class);
                put(SituationEventTrigger.TYPE, SituationEventTrigger.class);
            }
        };
    }
}
